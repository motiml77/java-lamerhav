...
## 14. ארגון מחדש של קבצי הפרויקט (נוסף לבקשת המשתמש)

### 14.1 המצב היום — והבעיה האמיתית

כל האפליקציה היא **קובץ אחד**: `app.html`, 972KB, 10,996 שורות. בתוכו: CSS, כל
קומפוננטות React, כל התוכן הלימודי, כל 8 ההדגמות האינטראקטיביות, שכבת הנתונים
והפרומפטים ל־AI.

הבעיה אינה רק אסתטית. הדף טוען React, ReactDOM, **Babel Standalone** ו־Tailwind
מ־CDN, ואז **מהדר בדפדפן 11,000 שורות JSX — בכל טעינה מחדש, אצל כל תלמידה**.
זו העלות היחידה הגדולה ביותר בזמן הטעינה, והיא מורגשת במיוחד בנייד (ולא במקרה
הקומיט האחרון ב־`master` הוא "Make the teacher UI usable on phones").
שלב בנייה מסלק את ההידור הזה לגמרי — הדפדפן מקבל JS מוכן, מוקטן ומפוצל.

*(המספרים המדויקים לא נמדדו — כדאי למדוד ב־Lighthouse לפני ואחרי.)*

### 14.2 האפשרויות

| | גישה | יתרון | חיסרון |
|---|---|---|---|
| א | **Vite + קבצי מקור** | ארגון אמיתי; אין הידור בדפדפן — הדף נטען מהר יותר; ייבוא רגיל; אפשר לינטר ובדיקות | צריך `npm run build`; אי אפשר עוד לפתוח את הקובץ ישירות בדפדפן |
| ב | פיצול בלי שלב בנייה | הקוד מסודר; עדיין "פותחים קובץ ורואים אתר" | ההידור בדפדפן נשאר — הבעיה המרכזית לא נפתרת; אף מחמירה (עוד קבצים להדר) |
| ג | להשאיר קובץ אחד | אין סיכון | הבעיה מחמירה ככל שהקוד גדל |

**המלצה: אפשרות א' (Vite).** היא היחידה שפותרת גם את הסדר וגם את הביצועים.
Vercel בונה פרויקט Vite מקומית ללא הגדרות מיוחדות.

### 14.3 מבנה היעד המוצע

```
├── index.html                  ← דף הנחיתה (נשאר)
├── public/                     ← נכסים המוגשים כמו שהם
│   ├── background.png
│   └── codemirror-java.min.js
├── src/
│   ├── main.jsx                ← נקודת כניסה + ניתוב
│   ├── config/
│   │   ├── firebase.js         ← firebaseConfig, init
│   │   └── constants.js        ← SUPER_ADMIN_EMAIL, WORKER_URL, פרומפטים
│   ├── routing/
│   │   └── resolveRoute.js     ← פענוח slug / admin / שורש
│   ├── services/
│   │   ├── DataService.js      ← כל הגישה ל-Firestore (היום: 841–1547)
│   │   ├── SchoolService.js    ← חדש: טעינה/יצירה/השהיה של בתי ספר
│   │   └── AIService.js        ← היום: 1553–1654
│   ├── content/                ← תוכן לימודי סטטי (כמעט 2,000 שורות)
│   │   ├── learnLibrary.js     ← 1656–1810
│   │   ├── topicLessons.js     ← 1811–1960
│   │   ├── dataStructuresHelp.js ← 5015–5168
│   │   ├── codeSnippets.js     ← 5169–5434
│   │   └── cheatSheet.js       ← 6330–6427
│   ├── components/
│   │   ├── common/             ← Icon, Modal, ActionMenu, LiveClock, ExamTimer
│   │   ├── visualizers/        ← 8 ההדגמות (2075–3400): רשימה מקושרת, מחלקות,
│   │   │                          רקורסיה, מערכים, תור/מחסנית, מחרוזות, עץ בינארי, קלט־פלט
│   │   └── editor/             ← עורך הקוד והמעטפת שלו
│   ├── pages/
│   │   ├── LoginScreen.jsx     ← 3924+
│   │   ├── StudentApp.jsx
│   │   ├── TeacherApp.jsx
│   │   └── AdminConsole.jsx    ← חדש (סעיף 8)
│   └── styles/
├── firebase/
│   ├── firestore.rules
│   └── scripts/                ← seed.js, migrate-to-schools.js
├── docs/
├── firebase.json · vercel.json · package.json
```

הפיצול טבעי: התוכן הלימודי וההדגמות הם כ־45% מהקובץ והם כמעט בלתי־תלויים בשאר.

### 14.4 סדר מומלץ: ריבוי בתי ספר **קודם**, ארגון מחדש **אחר כך**

זו ההמלצה החשובה כאן, והנימוק הוא ניהול סיכונים:

- **הארגון מחדש הוא בעיקר *העברת* קוד** — לכן כמעט לא משנה אם הוא קורה לפני
  או אחרי. אין כאן "עבודה כפולה" משמעותית.
- **המעבר לריבוי בתי ספר הוא כבר מהלך עדין**: מיגרציית נתונים חיה + החלפת חוקי
  אבטחה. להוסיף לאותו חלון גם החלפת מערכת בנייה שלמה — מכפיל את רדיוס הנזק.
  אם משהו נשבר, לא נדע מה מהשניים גרם.
- **הארגון מחדש בטוח לאימות**: הוא לא אמור לשנות שום התנהגות, ולכן קריטריון
  הקבלה שלו חד וברור — "האתר נראה ומתנהג בדיוק אותו דבר".

לכן: קודם שלבים 1–6 (ריבוי בתי ספר), ואחריהם שלב 7 — הארגון מחדש.

**כדי שהארגון יהיה קל אחר כך**, כל קוד חדש שנכתב עכשיו ייכתב "מוכן לפיצול":
בלוקים עצמאיים, בלי תלות סמויה בהיקף החיצוני, עם סימון ברור בהערות — כך
שהפיצול יהיה גזירה־והדבקה ולא התרה של סבך.

### 14.5 שלב 7 — הארגון מחדש (בסיום)

| תת־שלב | תוכן | קריטריון קבלה |
|---|---|---|
| 7א | `package.json` + Vite + React כתלויות אמיתיות (במקום CDN) | `npm run dev` מריץ את האתר כמו שהוא |
| 7ב | חילוץ `content/` — התוכן הלימודי (ללא לוגיקה) | האתר זהה |
| 7ג | חילוץ `components/visualizers/` | כל 8 ההדגמות עובדות |
| 7ד | חילוץ `services/` ו־`config/` | טעינה ושמירה עובדות |
| 7ה | חילוץ `pages/` — פיצול `App` לתלמידה/מורה/מנהל | כל שלושת המצבים עובדים |
| 7ו | הגדרת בנייה ב־Vercel + מדידת Lighthouse לפני/אחרי | האתר מהיר יותר, לא איטי יותר |

כל תת־שלב = קומיט נפרד שאפשר לחזור ממנו.

---

*המסמך נכתב על בסיס קריאת הקוד הקיים בענף `master` (קומיט `41a9bbf`). מפת הקוד המדויקת — ב־`docs/implementation-notes.md`. לאחר אישור ההחלטות הפתוחות — מתחילים בשלב 1.*
